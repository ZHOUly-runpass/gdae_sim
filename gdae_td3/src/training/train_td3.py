...
class TD3Trainer:
    ...
    def _get_state(self, obs, last_action=None):
        ...
        Args:
            obs: 环境观测
            last_action: 上一次动作 [linear, angular]，范围 [-1, 1]
        ...
        # 拼接状态：[laser(20), distance(1), angle(1), last_action(2)]
        state = np.concatenate([
            laser_compressed,
            obs['robot_state'],  # [distance, angle]
            last_action  # [action[0], action[1]]，范围 [-1, 1]
        ])
    ...
    def train(self):
        ...
        obs = self.env.reset()
        last_action = np.array([0.0, 0.0])
        state = self._get_state(obs, last_action=last_action)  # 修改
        ...
        while self.total_timesteps < self.config['max_timesteps']:
            ...
            # 构建下一个状态（使用环境返回的速度信息）
            next_state = self._get_state(next_obs, last_action=action)
    ...
    def evaluate(self, num_episodes=10):
        ...
        for _ in range(num_episodes):
            ...
            while not done and steps < self.config['max_episode_steps']:
                ...
                next_obs, reward, done, info = self.env.step(action_in)
                next_state = self._get_state(next_obs, last_action=action)  #  修改
